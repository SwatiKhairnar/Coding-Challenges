/*
Given an array of scores, compute the int average of the first half and the second half, and return whichever is larger. 
We'll say that the second half begins at index length/2. The array length will be at least 2. 
To practice decomposition, write a separate helper method 
int average(int[] scores, int start, int end) { which computes the average of the elements between indexes start..end. 
Call your helper method twice to implement scoresAverage(). Write your helper method after your scoresAverage() method 
in the JavaBat text area. Normally you would compute averages with doubles, but here we use ints so the expected results are exact.


scoresAverage([2, 2, 4, 4]) → 4
scoresAverage([4, 4, 4, 2, 2, 2]) → 4
scoresAverage([3, 4, 5, 1, 2, 3]) → 4
*/

object _04scoresAverage{
    def scoresAverage(scores:Array[Int]):Int = {
        val first = average(scores,0,scores.length/2)
        val second = average(scores,scores.length/2, scores.length)
        Math.max(first,second)
    }
    def average(scores:Array[Int], start:Int, end:Int):Int = {
        var sum = 0
        for(i <- start until end){
            sum += scores(i)
        }
        sum / (end - start)
    }
    

    def main(args: Array[String]) {
        println("scoresAverage([2, 2, 4, 4]) -> " + scoresAverage(Array(2, 2, 4, 4)));
        println("scoresAverage([4, 4, 4, 2, 2, 2]) -> " + scoresAverage(Array(4, 4, 4, 2, 2, 2)) );
        println("scoresAverage([3, 4, 5, 1, 2, 3]) -> " + scoresAverage(Array(3, 4, 5, 1, 2, 3)) );
    }
}
/*
scoresAverage([2, 2, 4, 4]) -> 4
scoresAverage([4, 4, 4, 2, 2, 2]) -> 4
scoresAverage([3, 4, 5, 1, 2, 3]) -> 4
*/